students = [
    ...
    {"Имя": "Иван", "Фамилия": "Филяев", "Пол": "М", "Опыт": 1, "ДЗ": [8, 9, 10, 8, 10], "Экзамен": 9},
...
def x(stud, sex=None, exp=None):  # Средняя по ДЗ
    ...
    return round(float(sum(a) / len(a)), 1)
...
def best_st(stud):
    ...
    strings = "Лучшие студенты: "
    if len(a) > 1:
        strings = "Лучшие студенты: "
    else:
        strings = "Лучший студент: "
    for st in a:
        strings += "{0} {1} с интегральной оценкой: {2}, ".format(st["Имя"], st["Фамилия"], st["integral_mark"])
    print(strings[:-2])
...
print("Средняя за ДЗ у студентов с опытом:", x(students, exp=1), "Средняя за экзамен у студентов с опытом:",
      y(students, exp=1))
print("Средняя за ДЗ у студентов без опыта:", x(students, exp=0), "Средняя за экзамен у студентов без опыта:",
      y(students, exp=0))
best_st(students)
